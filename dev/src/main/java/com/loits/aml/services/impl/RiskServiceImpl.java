    ...
    public CompletableFuture<?> calculateRiskForCustomerBase(String user, String tenent) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                // this behaviour.
                if (env.getProperty("aml.risk-calculation.segments") != null && !env.getProperty("aml.risk-calculation.segments").equals("-1")) {
                    // found page number overriding values
                    noOfAsyncTasks = Integer.valueOf(env.getProperty("aml.risk-calculation.segments"));
                    isDebugMode = true;
